...
def insert_into_db(db, sensore):
    value = round(uniform(-10, 45),2)
    complete_value = {'sensor': sensore, 'value': value}
...
def in_predicate(db):
    collection = db['sensors']
    seq = ["temp1", "temp2"]; # This is sequence of strings.
    all_sensors = {}
    count = 0
    for sensor in collection.find({"_id":{"$in": seq}}):
        if(sensor['type'] == 'temp'):
            all_sensors[count] = sensor
            print("temp")
        count = count + 1
